...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    const aliceDepositAmount = utils.parseEther('0.1') // Alice deposits 0.1 ETH in L1
    ...
    await token.transfer(omniBridge.address, aliceDepositAmount)// simulates bridge,, first sends tokens to omnibridge, then to pool
    ...
    const aliceWithdrawAmount = utils.parseEther('0.08')// Alice withdraws 0.08
...
it('[assignment] iii. see assignment doc for details', async () => {
    ...
    const aliceDepositAmount = utils.parseEther('0.13') // Alice deposits 0.13 ETH in L1
    const aliceDepositUtxo = new Utxo({ amount: aliceDepositAmount,  keypair: aliceKeypair })
    ...
    const bobSendAmount = utils.parseEther('0.06') // Alice sends 0.06 ETH to Bob in L2
    ...
    const bobEthAddress = '0xDeaD00000000000000000000000000000000BEEf' // Bob withdraws all his funds in L2
    const bobChangeUtxo = new Utxo({ amount: bobSendAmount.sub(bobReceiveUtxo.amount), keypair: bobKeypair })
    await transaction({
      tornadoPool,
    ...
    const aliceEthAddress = '0xaA6Ce349b3EE3129846ec71a81EFFfe57F5dD37B' // Alice withdraws all his funds in L2
    const aliceWithdrawalUtxo = new Utxo({ amount: aliceDepositAmount.sub(bobSendAmount).sub(aliceDepositAmount.sub(bobSendAmount)), keypair: aliceChangeUtxo.keypair })
    await transaction({
      tornadoPool,
    ...
    expect(aliceBalance).to.be.equal(utils.parseEther('0.07'))
